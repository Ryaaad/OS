...
import { FocusEvent, JSXElementConstructor, ReactElement, ReactFragment, ReactPortal, useState } from 'react';
...
const AddCtrl:React.FC<props> = (props) => {
  ...
  const [data, setdata] = useState( Type )
   ...
   const handleFocus=(Type:Number,e: FocusEvent<HTMLInputElement, Element>,id?:number)=>{
        ...
        <div className="relative">
           ...
           ></Image>
            <input type="text" onFocus={(e)=>{handleFocus(0,e)}} value={CustomerField &&  CustomerField?.nomC + ' '+CustomerField?.prenomC}
             onBlur={(e)=>{handleBlur(0)}} onChange={(e)=>handleChange(e,1)} placeholder="Select Customer" 
            ...
            return    <div className='flex gap-4' key={Math.random()} >
               ...
               {  Products && <div  className='border border-solid text-black border-black absolute bottom-[-400%] w-[300px] overflow-y-scroll h-[150px] max-h-[150px] bg-white ' >
                    {ProductsData  && ProductsData.map((Data:any)=>{
                     return <div  className='flex p-2 cursor-pointer gap-1 border-b-solid border-b border-b-black' key={Math.random()}
                     onClick={()=>{setProductField((prev:any)=>prev=Data);  }}> 
                            <p>N° {Data.codeP} </p> 
                            <p> {Data.nomP} </p>
